...
/// Roll back a `docker run` that was started but could not be adopted as a
/// Harness Hat session. Once Docker has accepted `run`, errors in the PTY or
/// cidfile handshake must never leave the container running without its
/// scoped proxy and manager-owned terminal.
///
/// The guard is deliberately armed before opening the PTY, because opening it
/// launches Docker. It also covers unwinding panics in the post-launch
/// handshake. A successfully constructed [`ContainerSession`] disarms it.
struct DockerRunRollback {
    docker_name: String,
    armed: bool,
}

impl DockerRunRollback {
    fn new(docker_name: String) -> Self {
        Self {
            docker_name,
            armed: true,
        }
...
impl Drop for DockerRunRollback {
    fn drop(&mut self) {
        if !self.armed {
            return;
        }

        // The name is generated by Harness Hat and never incorporates a user
        // value. `docker rm --force` is idempotent: a failed `docker run` or
        // Docker's own `--rm` cleanup simply produces a harmless not-found.
        let mut command = std::process::Command::new("docker");
        crate::process_util::hide_console_window(&mut command);
        match command
            .args(["rm", "--force", &self.docker_name])
            .stdout(std::process::Stdio::null())
            .stderr(std::process::Stdio::piped())
            .output()
        {
            Ok(output) if output.status.success() => {
                warn!(container = %self.docker_name, "removed unadopted container after launch failure");
            }
            Ok(output) => {
                let stderr = String::from_utf8_lossy(&output.stderr);
                // A container that has already disappeared is the desired
                // state, and Docker's wording is stable enough to recognize
                // without treating it as a second launch failure.
                if !stderr.to_ascii_lowercase().contains("no such container") {
                    warn!(
                        container = %self.docker_name,
                        status = ?output.status.code(),
                        stderr = %stderr.trim(),
                        "failed to remove unadopted container after launch failure"
                    );
                }
            }
            Err(error) => {
                warn!(
                    container = %self.docker_name,
                    %error,
                    "could not start rollback cleanup for unadopted container"
                );
            }
        }
    }
...
pub fn spawn(
    ...
    // `tty::new` is the moment Docker is actually launched. From this point
    // through successful session construction, rollback owns the container if
    // any subsequent operation fails.
    let mut rollback = DockerRunRollback::new(docker_run_name.clone());
